...
C_F_MF_Y <- sourceData %>%                                          
  ...
  slice_max(n=3, order_by=n, with_ties=F) %>%                                  # Keeping only the top 3 most bought items per familly
  merge(Produits %>% select(ProduitId, Libelle), by="Libelle", sort=F) %>%     # Matching Libelle to its ProductId
